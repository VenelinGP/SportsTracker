...
@objc class Calculations : NSObject{
    var property:String = ""
    func metthod(){
        print(self.property)
    }
    ...
    func stringifyDistance(inMeters meters: Float) -> String {
    ...
    func stringifySecondCount(withSeconds seconds: Int, andLongFormat longFormat: Bool) -> String {
    ...
    func stringifyAvgPaceFromDist(inMeters meters: Float, andSeconds seconds: Int) ->String{
    ...
    func stringifyAvgSpeed(inMeters meters: Float, andSeconds seconds: Int) ->String{
        if (seconds == 0 || meters == 0) {
            return "0";
        }
        
        let avgSpeedSecMeters: Float  = meters/Float(seconds);
        ...
        let avgSpeedMiPerSec: Int = (Int) ((avgSpeedSecMeters * unitMultiplier) / 60);
        
        return String(format:"%i %@", avgSpeedMiPerSec, unitName);
